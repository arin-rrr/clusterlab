...
async def create_user(user: UserCreate, db: AsyncSession = Depends(get_async_db)):
    result_email = await db.scalars(select(UserModel).where(UserModel.email == user.email))
    existing_user = result_email.first()
    ...
    if existing_user:
        if existing_user.is_verified:
            raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail='Email already exists')
        # Пользователь уже начинал регистрацию, но не подтвердил — обновляем данные и код
        existing_user.hashed_password = hash_password(user.password)
        existing_user.full_name = user.full_name
        existing_user.verification_code = code
        existing_user.verification_code_expires = datetime.utcnow() + timedelta(minutes=15)
